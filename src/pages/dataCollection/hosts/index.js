import { Button, Descriptions, Modal, Space, Table, Tag, Tooltip, TreeSelect } from "antd";
    ...
    <>
      <Modal
        ...
        onOk={async () => {
          ...
          const template = await templateService.getTemplatesByHost(selectedRowKeys[0])
          console.log(template);
          const templateids = template.result.map(template => template.templateid)
          console.log(templateids);
          console.log(value)
          console.log([...templateids, value]);
          const response = await hostService.updateHostTemplates(selectedRowKeys[0], [...templateids, value]);
          if (response.error) {
        ...
        <Button
          ...
          onClick={async () => {
            ...
            const treeDataTemp = templateGroups.result.map(async (templateGroup) => {
              const templates = await templateService.getTemplatesByTemplateGroup(templateGroup.groupid);
